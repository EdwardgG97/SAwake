...
create table FALTAS_ALERTAS(
id_alertas int primary key auto_increment,
detalle_alertas varchar (255) not null,
fecha_alertas datetime not null,
tipo_id int,
doc_id int,
est_id int,
acu_id int,
foreign key (tipo_id) references TIPOS_FALTAS (id_tipo),
foreign key (doc_id) references  DOCENTES (id_doc),
foreign key (est_id) references ESTUDIANTES (id_est),
foreign key (acu_id) references ACUDIENTES (id_acu)
);

create table CORRECTIVOS_FALTAS(
id_correctivo int primary key auto_increment,
detalle_correctivo varchar (255) not null,
paso_correctivo varchar (2) not null,
tipo_id int,
foreign key (tipo_id) references TIPOS_FALTAS (id_tipo)
);
